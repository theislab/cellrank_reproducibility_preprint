...
def _run_cellrank(adata: AnnData,
                  metast: Optional[int],
...
def run_analysis(adata: AnnData,
    ...
    str_pars = f'knn_{n_neighbors}_npcs_{n_pcs}_percd_{perc_data}_seed_{seed}_nhvg_{n_top_genes}_minsc_{min_shared_counts}_nstat_{n_metast_fwd}_velo_uncert'
    ...
    if cache_object is not None:
        cache_object.tl.recover_dynamics(adata_comp, fname=f'pars_{str_cache}', force=force)
    else:
    ...
    if show_figures:
        figure_params = {'color': ['clusters'], 'title': f'scvelo velocities',
                         'legend_loc': 'right'}
        if save_figures: figure_params['save'] = f'velocities_{str_pars}.png'
    ...
    if show_figures:
        figure_params = {'title': f'initial and terminal states', 'legend_loc': 'right', 'size': 100}
        if save_figures:
    ...
    dict_6 = {f'meta_{i}_fwd_err': value for i, value in enumerate(fev_fates_meta_error)}
    ...
    params = {'n_metas_fwd': n_metast_fwd,
...
def _pairwise_non_aligned_corr(g1: BaseEstimator,
                               g2: BaseEstimator,
                               attr: str,  # either metastable_states or absorption_probabilities
                               key: str):
    """Utility function to handle the case of subsampled data."""

    o1, o2 = getattr(g1, attr).copy(), getattr(g2, attr).copy()

    mask_1 = [state.startswith(key) for state in o1.names]
    mask_2 = [state.startswith(key) for state in o2.names]

    assert (np.sum(mask_1) == 1), f"`{key}` not (uniquely) in g1"
    assert (np.sum(mask_2) == 1), f"`{key}` not (uniquely) in g2"
    o1.names[mask_1] = key
    o2.names[mask_2] = key

    # construct pandas DataFrames from both
    l1 = pd.DataFrame(o1, index=g1.adata.obs_names, columns=o1.names)
    l2 = pd.DataFrame(o2, index=g2.adata.obs_names, columns=o2.names)

    # joint the df's, using 'inner' and index-to-index
    joined_df = l1.join(l2, lsuffix='_l', rsuffix='_r', how='inner')
    # print(f'WARNING: Subsetting lineages to their overlap ({joined_df.shape[0]} cells)')
...
def plot_correlation_map(data: Dict[Union[str, float], BaseEstimator],
                         axes = None,
                         show_title: bool = True,
                         attr: str = "absorption_probabilities",
                         keys: Tuple = ('Alpha', 'Beta', 'Epsilon', 'Delta'),
                         tri: bool = True,
    ...
    **kwargs
    ...
    if axes is None and not return_ranges:
        fig, axes = plt.subplots(1, len(keys), figsize=(len(keys) * 6, 6),
                                 sharey="row",
                                 constrained_layout=True)
        # fig.tight_layout()
        
    #assert len(keys) == len(axes) + 1
    ...
    for key in keys:
        ...
        if barcodes_align:
            ...
            for (j, i), (g1, g2) in zip(combinations(range(len(data)), 2), combinations(data.values(), 2)):
                corr_mat[i, j] = _pairwise_non_aligned_corr(g1, g2, attr=attr, key=key)
            corr_mat = corr_mat.T
    ...
    for i, (key, ax, corr_mat) in enumerate(zip(keys, axes, corr_mats)):
        ...
        sns.heatmap(corr_mat, vmin=vmin, vmax=vmax,
                    cmap=cmap, ax=ax, mask=mask, **kwargs,
        ...
        assert np.all(~quant_mask[np.arange(quant_mask.shape[0])[::-1], np.arange(quant_mask.shape[0])])
        
        ax.text(0.05, 0.9, f'med = {np.median(corr_mat.values[quant_mask]):.2f}', horizontalalignment='left',
                verticalalignment='center', transform=ax.transAxes)
        ax.text(0.05, 0.8, f'min = {np.min(corr_mat.values[quant_mask]):.2f}', horizontalalignment='left',
                verticalalignment='center', transform=ax.transAxes)
    ...
    if show_cbar:
        ...
        cax = inset_axes(ax, width="5%", loc="right", height="100%",
        ...
        _ = matplotlib.colorbar.ColorbarBase(ax=cax, norm=norm, cmap=cmap, label="correlation", ticks=cticks, format="%.3f")
...
def _create_root_final_annotations(
    ...
    if f"{final_key}_colors" not in adata.uns:
        adata.uns[f"{final_key}_colors"] = _create_categorical_colors(len(adata.obs[final_key].cat.categories))

    if f"{root_key}_colors" not in adata.uns:
        adata.uns[f"{root_key}_colors"] = _create_categorical_colors(30)[::-1][len(adata.obs[root_key].cat.categories):]
