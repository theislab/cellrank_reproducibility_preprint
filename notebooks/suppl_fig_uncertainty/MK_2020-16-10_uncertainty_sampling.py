...
if os.path.isfile(ROOT / "adata_preprocessed.h5ad") and os.path.isfile(ROOT / "terminal_states.csv"):
    print("Loading data")
    ...
    k = cr.tl.transition_matrix(adata, weight_connectivities=0.2, mode="stochastic", n_jobs=n_jobs, softmax_scale=None,
...
def compute_abs_probs(ixs: np.ndarray, adata: AnnData, terminal_states: pd.Series, c: cr.tl.kernels.ConnectivityKernel):
    res = []
    
    for i in ixs:
        try:
            conn = c.copy()
            vk = cr.tl.kernels.VelocityKernel(adata.copy()).compute_transition_matrix(seed=i, n_jobs=2,
                                                                                      mode="sampling",
                                                                                      show_progress_bar=False,
                                                                                      backend="loky",
                                                                                      softmax_scale=None)
            ...
            g.compute_absorption_probabilities(use_petsc=True, n_jobs=2, show_progress_bar=False)
...
res = jl.Parallel(n_jobs=n_jobs, backend="loky")(jl.delayed(compute_abs_probs)(ixs, adata, terminal_states, c)
...
print("Saving")
with open(ROOT / "uncertainity.pickle"), "wb") as fout:
...
figure_kwargs = {'title': f'fate probabilities mean', 
...
figure_kwargs = {'title': f'fate probabilities var', 
                 'lineages': ['Alpha', 'Beta', 'Epsilon', 'Delta'],
                 'cluster_key': 'clusters_fine',
                 'clusters': clusters,
                 'save': FIG_DIR / "suppl_fig_uncertainity" / "variance.pdf",
                 'figsize': (5, 3),
                }
