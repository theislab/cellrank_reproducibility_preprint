...
def _benchmark_cellrank(bdata, size: int, col: int, Estimator, n_states: int = 3, n_jobs: int = 32):
    ...
    try:
        ...
        scv.tl.velocity_graph(bdata, mode_neighbors='connectivities', n_recurse_neighbors=0)
        ...
        if Estimator is cr.tl.estimators.GPCCA:
            ...
            apmem, _ = compute_ap(use_petsc=True, time_to_absorption=None, solver="gmres",
                               show_progress_bar=False,
                               n_jobs=n_jobs, backend="loky")

            drivemem, _ = compute_ld(return_drivers=True)
            ...
            print("Computing eidecomposition")
            ...
            print("Computing absorption probabilities")
            apmem = compute_ap(use_petsc=True, time_to_absorption=None, solver="gmres",
                               show_progress_bar=False,
                               n_jobs=n_jobs, backend="loky")
            res['eig_mem'] = eigmem
        ...
        print(f"Unable to run `{Estimator}` with size `{size}` on split `{col}`. Reason: `{e}`.")
...
# this is considered legacy code and not used in the benchmarks
def benchmark_cflare(adata: AnnData, size: int, col: int, n_jobs: int = 32):
    from utils import PROFILER_ROOT, PROFILER_ROOT_1_CORE
    
    path = PROFILER_ROOT_1_CORE if n_jobs == 1 else PROFILER_ROOT
    path = path / "cflare" / f"{size}_{col}.pickle"

    if not os.path.isfile(path.parent):
        os.makedirs(path.parent, exist_ok=True)

    res = _benchmark_cellrank(adata, size, col, cr.tl.estimators.CFLARE, n_jobs=n_jobs)
